...
class SupervisedTrainingDataset(TrainingDataset):
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor]:
        ...
        image = (transformed['image']*tissue_region + image*(1-tissue_region)).astype(np.uint8)
        # (5) apply color augmentation transformations to the non-tissue regions
        transformed = self.non_tissue_color_transforms(image=image)
        image = (transformed['image']*(1-tissue_region) + image*tissue_region).astype(np.uint8)
        ...
        image = (transformed['image']*pen_region + image*(1-pen_region)).astype(np.uint8)
        ...
        image = (transformed['image']*background + image*(1-background)).astype(np.uint8)
    ...
    def get_aug_transforms(self) -> list:
        ...
        for region in ['tissue', 'non-tissue', 'pen', 'background']:
